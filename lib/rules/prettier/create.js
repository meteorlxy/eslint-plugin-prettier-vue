  ...
  return {
    Program() {
      if (filepath.endsWith('.vue')) {
        // Handle Vue SFC
        const { script, styles, customBlocks } = parseVue({ source, filepath })

        // `<script>` block
        prettierDifferences({ context, ...script, options: prettierOptions })

        // `<style>` blocks
        styles.forEach(({ source, offset }) => prettierDifferences({ context, source, offset, options: prettierOptions }))

        // custom blocks
        customBlocks.forEach(({ source, offset }) => prettierDifferences({ context, source, offset, options: prettierOptions }))
      } else {
        // Handle Other files
